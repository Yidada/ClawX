...
開発モードでは、ClawX は OpenClaw の状態をこのチェックアウト配下の `./.openclaw/` に固定し、複数のローカルクローンが `~/.openclaw/` を共有しないようにします。
...
- **設定 → 詳細 → 開発者** では **OpenClaw Doctor** を実行でき、`openclaw doctor` の診断出力をアプリ内で確認できます。
